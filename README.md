...
- при старте runtime последовательно проверяет модели из `[openrouter].models` генерацией с промтом из `ai/prompts/model_probe.md`: «Ответь только словами: Да, работаю» через тот же OpenRouter SDK-клиент и общий построитель запроса, что используют боты (одна модель на попытку, общий deadline 8 секунд с учётом SDK retry). Логи `OpenRouter startup request/answer/result` показывают модель, проверочный промт, короткий ответ (до 80 символов, с редактированием секретов) и статус. Любой непустой текст подтверждает работу; после первого успеха проверки прекращаются и запуск продолжается. Только если все модели не ответили текстом, загружается каталог бесплатных text-output моделей. Отчёт `logs/openrouter_free_models.json` содержит результаты попыток без сырого текста; при успехе — `catalog_fetched=false`, при получении каталога — список slugs и `toml_models_line` для ручного копирования. Конфигурация моделей автоматически не меняется;
