...
- **THEN** the orchestrator composes `start_topic` with group context, generates a non-repeating question, sends it to the resolved group target, marks the exchange started, and saves a `scheduled_initiator` message with the real group chat id
