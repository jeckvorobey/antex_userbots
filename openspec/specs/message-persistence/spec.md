...
The system SHALL create and migrate a `messages` table for persisted chat history.
...
The system SHALL create and migrate a `scheduled_exchanges` table for orchestrator state.

#### Scenario: Exchange store initialization creates storage
- **WHEN** `ExchangeStore.init_db` runs
- **THEN** the `scheduled_exchanges` table exists with fields for pair, window, topic, question, due timestamps, message ids, status, and lifecycle timestamps
...
The system SHALL persist planned, started, and completed scheduled exchange state.

#### Scenario: Planned exchange is created
- **WHEN** an exchange is created
- **THEN** it receives a UUID, pair key, topic key, optional window key, optional initiator due timestamp, and status `planned`
...
The system SHALL expose persisted queries used by orchestrator anti-repeat behavior.

#### Scenario: Recent bot ids follow scheduled message order
- **WHEN** completed and started exchanges exist
- **THEN** recent bot id retrieval returns unique bot ids in recent scheduled message order

#### Scenario: Recent topic keys are limited
- **WHEN** more topic keys exist than the requested limit
- **THEN** only keys from the latest started or completed exchanges up to the limit are returned

#### Scenario: Recent question signatures are normalized
- **WHEN** question signatures are stored with punctuation or spacing
- **THEN** anti-repeat queries use normalized lowercase signatures without punctuation

#### Scenario: Due started exchange is returned
- **WHEN** a started exchange has a responder due timestamp not later than now
- **THEN** `get_due_started_exchange` returns that exchange record
