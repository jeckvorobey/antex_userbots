...
- `MessageHistory` и `ExchangeStore` используют одно SQLite-соединение и общий transaction lock на весь runtime;
- проект старается не повторять одни и те же вопросы слишком часто;
- ключевые действия подробно пишутся в лог.

Ошибки генерации OpenRouter записываются на уровне `ERROR`, поэтому строка
`OpenRouter generation failed` попадает в файловый лог с `file_level = "ERROR"`.
Она содержит категорию, HTTP status и доступные безопасные коды ошибки из ответа SDK.
Сырой ответ провайдера, промты и секреты не записываются; отсутствующие поля обозначаются `unknown`.
...
Важные service-вопросы встроены в тот же плановый режим:
...
- при старте runtime последовательно проверяет модели из `[openrouter].models` генерацией с промтом из `ai/prompts/model_probe.md`: «Ответь только словами: Да, работаю» через тот же OpenRouter SDK-клиент и общий построитель запроса, что используют боты (одна модель на попытку, общий deadline равен `openrouter.request_timeout_seconds` (по умолчанию 45 секунд) с учётом SDK retry). Логи `OpenRouter startup request/answer/result` показывают модель, проверочный промт, короткий ответ (до 80 символов, с редактированием секретов) и статус; неудачная попытка записывается с уровнем ERROR, а при отказе — безопасный код причины (`unsupported_parameters`, `data_policy`, `no_endpoints`, `unknown`). Любой непустой текст подтверждает работу; после первого успеха проверки прекращаются и запуск продолжается. Если все модели не ответили текстом, загружается диагностический каталог бесплатных text-output моделей, но запуск завершается с ошибкой до подключения Telegram и запуска scheduler. Успешный GET каталога не подтверждает генерацию; готовность явно обозначается `generation_available`. Ошибка записи отчёта после успешного ответа не отменяет готовность. Отчёт `logs/openrouter_free_models.json` содержит результаты попыток без сырого текста; при успехе — `catalog_fetched=false`, при получении каталога — список slugs и `toml_models_line` для ручного копирования. Конфигурация моделей автоматически не меняется. Запросы сохраняют `data_collection=deny`, но не включают `require_parameters=true`, который может отсеять рабочие endpoints по поддержке необязательных параметров;
- файл `logs/openrouter_free_models.json` не меняет рабочий список моделей автоматически и не содержит `OPENROUTER_API_KEY`;
- каждый запрос использует Chat Completions с `zdr=false`, `allow_fallbacks=true`, подсказкой `reasoning: {"effort": "low"}` и server-side пределом `max_completion_tokens=2048`; `finish_reason=length` логируется warning-ом, а `error`/`content_filter` бракуются как незавершённые ответы;
...
При каждом запуске приложение очищает прошлый persisted-снимок доступности и
...
- при инициализации каждой группы для каждого бота видно `can_write` и Telegram-параметры, по которым он определён: `participant_banned_rights.send_messages`, `default_banned_rights.send_messages`, `is_admin`.
...
Файловое логирование по умолчанию: `logs/swarm.log`, уровень `ERROR`. При запуске выводится абсолютный путь и порог файлового лога либо явное сообщение об отключении (`logging.file`). Стартовые отказы моделей попадают в этот файл.

При старте строка `Runtime configuration` показывает выбранный `SETTINGS_PATH`, модели и наличие общего `PROXY` без его адреса и credentials. Автоматического выбора `settings.prod.toml` по месту запуска нет: используется `SETTINGS_PATH` из окружения или `.env`, иначе `config/settings.toml`. Если в Docker подключён каталог `/app/config`, файлы из него заменяют конфигурацию из образа. При подключении `/app/logs` ошибки и `openrouter_free_models.json` доступны в соответствующей папке хоста.
